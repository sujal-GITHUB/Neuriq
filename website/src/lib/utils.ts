...
export function getAnxietyColor(level: string): string {
  switch (level) {
    ...
    case 'Both': return '#e11d48'; // Rose-red for comorbid anxiety and stress
...
export function getAnxietyBgClass(level: string): string {
  switch (level) {
    ...
    case 'Both': return 'bg-rose-500/20 text-rose-400 border-rose-500/30';
...
export const MOCK_PREDICTION: import('@/types').PredictionResult = {
  ...
  model_used: 'XGForest Stacking Ensemble',
